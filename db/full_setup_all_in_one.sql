...
BEGIN
  ...
  SELECT
    ...
    b.perfect,
    -- 勤學四態——跟結業達標是不同標準
    -- 2026-09-10 修正：原本「缺課總數 ≥ 4 就永久鎖定無法勤學」（2026-07-03 定案），
    -- 星星改口徑：只要目前手上沒有還沒補的（absent=0，全部補完），不管這學期總共缺過
    -- 幾次，都算「已勤學」，不再受「缺課總數 1～3」這個上限卡住，也不要求要結業。
    -- 「可勤學」／「無法勤學」的差別維持原樣：還有沒補完的（absent>0）時，缺課總數
    -- 在1～3內算「可勤學」（補完就變已勤學），超過3才是「無法勤學」。
    CASE
