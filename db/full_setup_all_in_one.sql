...
BEGIN
  ...
  SELECT
    ...
    b.perfect,
    -- 勤學三態——跟結業達標是不同標準
    -- 2026-09-10 第一次修正：原本「缺課總數 ≥ 4 就永久鎖定無法勤學」（2026-07-03 定案），
    -- 改成只要目前手上沒有還沒補的（absent=0，全部補完），不管這學期總共缺過幾次，都算
    -- 「已勤學」，不再受缺課總數上限卡住，也不要求要結業。
    -- 2026-09-10 第二次修正：星星進一步指出，既然補完就一定能拿到「已勤學」，那「可勤學」
    -- 跟「無法勤學」這兩個還沒補完時的中間狀態就沒有分開判定的必要了——尤其「無法勤學」
    -- 這個名字在新規則下根本不準確（它明明還是可以補完拿到已勤學，不是真的「無法」）。
    -- 因此把這兩態合併成單一的「尚未勤學」，不再分缺課總數 1～3 或 ≥4；還差幾堂待補這個
    -- 資訊改由前端直接顯示 absent 數字，不需要用不同的狀態名稱來分級。
    CASE
      WHEN (b.absent + b.makeup + b.ml_makeup) = 0                                    THEN '目前全勤'
      WHEN b.absent = 0                                                                THEN '已勤學'
      ELSE '尚未勤學'
    END                                                                        AS diligent,
